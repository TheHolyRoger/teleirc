...
class TeleIrc {
  ...
  initStage4_addIrcListeners() {
    ...
    this.ircbot.addListener('message', (from, channel, message) => {
      console.log("[" + channel + "::" + from + "] " + JSON.stringify(message));
  ...
  queueTelegramMessage(chatId, messageString) {
    const ircblocks = ircFormatting.parse(messageString).map(block => ({
      // Consider reverse as italic, some IRC clients use that
      ...block,
      italic: block.italic || block.reverse
    }));
    let message = '';
    for (let i = 0; i <= ircblocks.length; i += 1) {
      // Default to unstyled blocks when index out of range
      const block = ircblocks[i] || {};
      const prevBlock = ircblocks[i - 1] || {};
      message += block.text || '';
    }
    this.tgRateLimiter.queueMessage(message);
